...
def _detect_python(env: EnvReport) -> bool:
...
def detect_environment(env: EnvReport | None = None) -> EnvReport:
    ...
    if env.node_version:
        _row("Node.js 版本", _c(_BWHITE, env.node_version))
    _check_single(env, "npm", _detect_npm)
    if env.npm_version:
        _row("npm 版本", _c(_BWHITE, env.npm_version))
...
def install_dependencies(env: EnvReport, force: bool = False) -> bool:
    ...
    _row("安装目录", _c(_CYAN, str(env.desktop_dir)))
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            print(f" {_ide()}  请手动: {_c(_CYAN, 'cd desktop && npm install')}")
            return False
...
def _start_backend_in_thread(
    ...
    def _run():
        ...
        _set_progress("启动 uvicorn HTTP 服务器...")
        try:
            server.run()
        except Exception:
            pass
...
def _wait_backend_ready(host: str, port: int) -> int:
    ...
    # 等待 uvicorn started 事件，同时显示进度
    _row("启动端口", _c(_BCYAN, f"{host}:{port}"))
    _row("运行模式", _c(_BWHITE, "standard"))

    last_msg = ""
    while not _backend_ready.wait(timeout=0.15):
        with _progress_lock:
            msg = _progress_msg
        if msg != last_msg:
            _row("状态", _label(msg))
        ...
        if time.time() - start > BACKEND_STARTUP_TIMEOUT:
            _row("uvicorn", _err("✗ 启动超时"), False)
            return 0

    _row("uvicorn", _ok("✓ 已启动"), True)

    # HTTP 健康检查
    _row("健康检查", _label("轮询 /api/health ..."))
    ...
    while time.time() - start < BACKEND_STARTUP_TIMEOUT:
        ...
        try:
            ...
            if resp.status == 200:
                ...
                _row("模型服务", _c(_BWHITE, f"{svc} / {mdl}"))
                _backend_port = port
                return port
        except Exception:
            pass
...
def launch_ide(
    ...
    if not skip_install and not env.node_modules_exists:
        if not install_dependencies(env):
            ...
            return 1
        env = EnvReport()
        detect_environment(env)  # 重新检测以刷新路径
    else:
        _row("依赖状态", _ok("✓ 已安装"), True)
    ...
    if is_dev and not env.vite_dev_server_running:
        ...
        print(f" {_ide()}  请在另一终端执行: {_c(_CYAN, f'cd {env.desktop_dir} && npx vite')}")
        print(f" {_ide()}  后端已就绪: {backend_url}")
    ...
    if not electron_bin or electron_bin == "npx":
        _row("Electron 路径", _warn("未找到，尝试 npx"))
        electron_bin = "npx"
        electron_args = ["electron"]
    else:
        _row("Electron 路径", _c(_BWHITE, electron_bin))
        electron_args = []
    ...
    if eflags:
        _row("窗口协议", _ok("Wayland"))
    else:
        _row("窗口协议", _label("X11 / 默认"))
    ...
    try:
        ...
        print(f" {_ide()}  请运行: {_c(_CYAN, 'cd desktop && npm install')}")
        return 1
    except KeyboardInterrupt:
        print(f"\n {_ide()} {_label('已退出。')}")
        return 0
