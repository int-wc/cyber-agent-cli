...
GATEWAY_DEFAULT_MODEL=deepseek-v4-flash-free
...
- `OPENCODE_PROXY_URL`：必填，OpenCode Zen 专属代理，需要按本机网络环境替换为可用地址。也支持 `socks://proxy.example:7892`（会按 `socks5://` 处理），需安装 socks 支持依赖。
...
- `/service <服务商>` 会使用 `.env` 中的服务商专属基址或 `GATEWAY_BASE_URL`，不会改写 `.env`。
